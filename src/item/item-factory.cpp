...
namespace castlecrawl::item
{
    ...
    void ItemFactory::throwIfInvalid(const Item & item) const
    {
        ...
        if (item.isArmor())
        {
            ...
            M_CHECK(
                item.isEquipable(), "Error:  Armor Item SHOULD be Equipable but is not: " << item);
        ...
        if (item.isWeapon())
        {
            M_CHECK((item.damageMin() > 0), "Error:  Weapon Item's damage_min invalid: " << item);
            ...
            M_CHECK(
                item.isEquipable(), "Error:  Weapon Item SHOULD be Equipable but is not: " << item);
        ...
        if (item.isMisc())
        {
            if (item.isUseable())
            {
                ...
                M_CHECK(
                    ...
                    "Error: Misc item Potions and Herbs MUST have the material 'Magic': " << item);
            }
            else
            {
                M_CHECK(
